...
from knowledge_bert.modeling import BertForEntityTypingDescrip
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer_label, tokenizer, threshold,
    entity_id2parents, entity_id2label, max_parent, qid2idx, verbose=2):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        real_ents = [["UNK"]*max_parent] + entities + [["UNK"]*max_parent]
      ...
      for input_ids, input_mask, segment_ids, input_ent, ent_mask, labels in eval_dataloader:
          ...
          with torch.no_grad():
              tmp_eval_loss = model(input_ids, segment_ids, input_mask, input_ent,
                  ent_mask, labels, tokenizer=tokenizer, qid2idx=qid2idx, entity_id2label=entity_id2label, use_ent_emb=(not args.no_descrip))
              logits = model(input_ids, segment_ids, input_mask, input_ent,
                  ent_mask, labels=None, tokenizer=tokenizer, qid2idx=qid2idx, entity_id2label=entity_id2label, use_ent_emb=(not args.no_descrip))
